...
@implementation SCLogisticsDetailCell
...
@implementation SCLogisticsDetailInfoCell
...
-(void)refreshWithLogistMsg:(CKCLogistInfoModel*)infoModel {
    NSString *info = [NSString stringWithFormat:@"%@", infoModel.info];
    NSString *time = [NSString stringWithFormat:@"%@", infoModel.time];
    if (IsNilOrNull(info)) {
        info = @"";
    }
    if (IsNilOrNull(time)) {
    ...
    _latestLogistLable.text = [NSString stringWithFormat:@"%@\n%@", info, time];
}
